...
public class VideoRepository : BaseRepository, IVideoRepository
{
    ...
    public async Task<IList<Video>> SearchAsync(string search, PaginationParams @params)
    {
        try
        {
            await _connection.OpenAsync();
    ...
    public async Task<int> UpdateAsync(long id, Video entity)
    {
        try
        {
            ...
            string query = "UPDATE public.videos SET category_id=@CategoryId, name=@Name, description=@Description, " +
                "video_path=@VideoPath, created_at=@CreatedAt, updated_at=@UpdatedAt WHERE id = {id};";
